		...
		************************************************************-->
			<div role="tabpanel" class="tab-pane" id="review"
						...
						<div class="form-group">
							...
							<div class="progress" id="progress_1">
								<div id="total" class="progress-bar progress-bar-warning"
									...
									aria-valuemax="100" style="width: ${avg_all*20 }%">
									<span class="sr-only">70% Complete</span>
								</div>
							</div>
						</div>
					</div>
					<div class="col-sm-2" style="padding-top: 25px;">${avg_all }
						/ 5</div>
						...
						<div class="form-group">
							...
							<div class="progress" id="progress_1">
								<div id="possibility" class="progress-bar" role="progressbar"
									...
									style="width: ${avg.avg_possibility*20 }%">
									<span class="sr-only">70% Complete</span>
								</div>
							</div>
						</div>
					</div>
					<div class="col-sm-2" style="padding-top: 25px;">${avg.avg_possibility }
						/ 5</div>
				</div>
				...
				<div class="row"
					...
					<div class="col-sm-2" style="padding-top: 25px;">${avg.avg_welSal }
						/ 5</div>
						...
						<div class="form-group">
							...
							<div class="progress" id="progress_1">
								<div id="balance" class="progress-bar" role="progressbar"
									aria-valuenow="90" aria-valuemin="0" aria-valuemax="100"
									style="width: ${avg.avg_balance*20 }%">
									<span class="sr-only">70% Complete</span>
								</div>
							</div>
						</div>
					</div>
					<div class="col-sm-2" style="padding-top: 25px;">${avg.avg_balance }
						/ 5</div>
				</div>
				...
				<div class="row"
					...
					<div class="col-sm-2" style="padding-top: 25px;">${avg.avg_culture }
						/ 5</div>
						...
						<div class="form-group">
							...
							<div class="progress" id="progress_1">
								<div id="manager" class="progress-bar" role="progressbar"
									aria-valuenow="90" aria-valuemin="0" aria-valuemax="100"
									style="width: ${avg.avg_manager*20 }%">
									<span class="sr-only">70% Complete</span>
								</div>
							</div>
						</div>
					</div>
					<div class="col-sm-2" style="padding-top: 25px;">${avg.avg_manager }
						/ 5</div>
				</div>
			</div>
		</div>
	...
	************************************************************-->
	<div class="container" style="background-color: #ffffff;">
				...
				<table class="table table-hover">
					...
					<tbody>
						<c:forEach items="${list }" var="review" varStatus="stat">
							<tr>
								<td style="width: 10%;"><a
									href="control.do?action=select&no=${review.review_no }">${review.review_no }</a></td>
								<td style="width: 50%;"><a
									href="control.do?action=select&no=${review.review_no }">${review.review_title }</a></td>
								<td style="width: 10%;"><a
									href="control.do?action=select&no=${review.review_no }">${review.review_writer }</a></td>
								<td style="width: 10%;"><a
									href="control.do?action=select&no=${review.review_no }">${review.review_wdate }</a></td>
								<td id="count" style="width: 10%;"><a
									href="control.do?action=select&no=${review.review_no }">${review.review_count }</a></td>
		...
		이전
			...
			<c:if test="${startPage>5 }">
				<a
					href="control.do?company_id=${cominfo.company_id }&page=${startPage-5 }">이전</a>
			</c:if>
			<c:forEach begin="${startPage }" end="${endPage }" var="i">
   	  		[<a
					href="control.do?company_id=${cominfo.company_id }&page=${i }">${i }</a>]
		</c:forEach>
			<c:choose>
				<c:when test="${endPage<totalPage }">
					<a
						href="control.do?company_id=${cominfo.company_id }&page=${startPage+5 }">다음</a>
				</c:when>
			...
			<div class="row">
				<div class="col-md-12">
					...
					<p class="lead mb-0" style="font-size: 1em;">
						편견없는 채용을 위한 커뮤니티를 만들겠습니다.<br> 대표이사: 조영환<br> 주소: 서울시 서초구
						서초동 엔코아빌딩 2층 증강현실클래스<br> 대표전화: 010-2762-7989<br> Email:
						airjoyh@naver.com
					</p>
				</div>
			</div>
