...
function showLoginout(msg) {// msg는 ${loginState }--> LoginAction에서 로그인하면 세션을
							// 'login'이라는 메시지를 셋어트리뷰트한다,
	...
	if (msg == "login") {// 로그인 상태일때!
		...
		// loginout.innerHTML = '<a data-toggle="modal"
...
function requestLoginCheck() {// 로그인을 했을때 실행
	...
	// login.do --> 로그인액션(login_id와 login_pass가 필요)--> login_id와 login_pass의 값을
	// 전달한다.(POST), resultLoginCheck함수 실행,
}
	...
	if (xhr.readyState == 4) {// 요청한 데이터(페이지를 전부 받았을 때)
		if (xhr.status == 200) {// 서버의 상태 체크
			...
			if (str == 'success') {// 로그인 성공했을 때
				showLoginout('login');// 메인뷰에서 showLoginout함수를 실행하고
										// 'login'($loginState)을 파라미터로 전달전달
...
/*
 * $("#openJoinForm").click(function() { $("#joinModal").modal();
 * $('#loginModal').modal('hide');//로그인 창 닫기!! 안된다 시발 //window.close(); });
 */
...
function requestIdCheck() {// 중복확인을 눌렀을때
	...
	// idCheck.do --> idCheckAction실행 전에--> idCheckActionForm을 찾아서 유효성 검사를 하고
	// (user_id값을 파라미터로 받아서)
	// idCheckAction실행!
}
...
function validateCheck() {// 자바 스크립트 유효성 검사
	...
	if (!user_idExp.test(f.user_id.value)) {
		...
		} else if (idCheck.indexOf('사용할 수 없는 아이디입니다.') >= 0) {// '사용할 수 없는
																// 아이디입니다.'가
																// 포함되어있다면
			alert('중복된 아이디입니다.');
